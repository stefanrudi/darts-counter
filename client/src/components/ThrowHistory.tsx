...
export function ThrowHistory({ players }: ThrowHistoryProps) {
  ...
  const throwsByTurn: { playerName: string; throws: Throw[]; totalScore: number }[] = []
  ...
  throwsByTurn.sort((a, b) => new Date(b.throws[0].timestamp).getTime() - new Date(a.throws[0].timestamp).getTime())
      ...
      <CardContent>
        <div className="space-y-4 max-h-[400px] overflow-y-auto">
          {throwsByTurn.length === 0 ? (
            <div className="text-center text-muted-foreground">No throws yet</div>
          ) : (
